...
{
    /** @var \swoole_server|\swoole_http_server */
    protected $sw;
...
//    }
    ...
    {
        if (!($protocol instanceof \Swoole\Protocol\IProtocol)) {
            throw new \Exception("[error] The protocol is not instanceof \\Swoole\\Protocol\\IProtocol");
        }
    ...
    {
        if ($this->setting['log_file'] && file_exists($this->setting['log_file'])) {
    ...
    public function handleError($code, $message, $file, $line)
    {
        if (error_reporting() & $code) {
            ...
            if (!class_exists('Swoole\\Exception\\ErrorException', false)) {
                require_once(dirname(dirname(__DIR__)) . '/Exception/ErrorException.php');
            }
    ...
    public function handleFatalError()
    {
        ...
        if (ErrorException::isFatalError($error)) {
            ...
            $this->log($log);
            if (!empty($this->currentFd)) {
                $this->sw->send($this->currentFd, $log);
            }
        }
    }
}